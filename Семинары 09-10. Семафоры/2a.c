...
int main()
{
  ...
  if ((key = ftok(pathname,0)) < 0) {
    printf("[error 09-2a.c] can\'t generate key\n");
    exit(-1);
  }

  if ((shmid = shmget(key, 3*sizeof(int), 0666|IPC_CREAT|IPC_EXCL)) < 0) {
    if (errno != EEXIST) {
      printf("[error 09-2a.c] can\'t create shared memory\n");
      exit(-1);
    } else {
      if ((shmid = shmget(key, 3*sizeof(int), 0)) < 0) {
        printf("[error 09-2a.c] can\'t find shared memory\n");
        exit(-1);
      }
      new = 0;
    }
  ...
  if ((array = (int *)shmat(shmid, NULL, 0)) == (int *)(-1)) {
    printf("[error 09-2a.c] can\'t attach shared memory\n");
    exit(-1);
  }

  if ((semid = semget(key, 1, 0666)) < 0) {
    if ((semid = semget(key, 1, 0666 | IPC_CREAT)) < 0) {
      printf("[error 09-2a.c] can\'t create semaphore set\n");
      exit(-1);
    }
    ...
    if (semop(semid, &mybuf, 1) < 0) {
      printf("[error 09-2a.c] can\'t wait for condition\n");
      exit(-1);
    }
  }
  ...
  if (shmdt(array) < 0) {
    printf("[error 09-2a.c] can\'t detach shared memory\n");
    exit(-1);
  }
