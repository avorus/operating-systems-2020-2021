...
int main() {
    ...
    if ((key = ftok(pathname, 0)) < 0) {
        printf("[error 11-2a.c] can't generate key\n");
        exit(-1);
    }

    if ((msqid = msgget(key, 0666 | IPC_CREAT)) < 0) {
        printf("[error 11-2a.c] can't get msqid\n");
        exit(-1);
    }
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        if (msgsnd(msqid, (struct msgbuf *) &mybuf, len, 0) < 0) {
            printf("[error 11-2a.c] can't send message to queue\n");
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        printf("[success 11-2a.c] message type = %ld, iinfo = %i\n", mybuf.mtype, mybuf.info.iinfo);
    }

    // не забываем про удаление, раньше оно происходило после получения LAST_MESSAGE
    msgctl(msqid, IPC_RMID, (struct msqid_ds *) NULL);
